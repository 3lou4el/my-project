...
def is_line_touching(p1, p2, center, radius):
    # Если линия — это просто точка (выстрела нет)
    if p1 == p2:
        return False
    ...
    # Находим проекцию точки центра на прямую
    u = ((center[0] - p1[0]) * dx + (center[1] - p1[1]) * dy) / mag_sq

    # Ограничиваем проекцию границами отрезка (от 0 до 1)
    # Если u < 0, ближайшая точка - начало, если u > 1 - конец
    if u < 0:
        closest = p1
    elif u > 1:
        closest = p2
    ...
    # Считаем расстояние от центра до ближайшей точки на линии
    ...
    for i in range(len(balls)):
        for j in range(i + 1, len(balls)):
            ...
            if dist < BALL_RADIUS * 2 and dist > 0:
                ...
                # Передача импульса (Идеально упругий удар 2D)
                dvx = b1.vx - b2.vx
                dvy = b1.vy - b2.vy
                ...
                # Применяем импульс с небольшой потерей энергии (0.95)
                b1.vx -= p * nx * 0.95
                b1.vy -= p * ny * 0.95
                b2.vx += p * nx * 0.95
                b2.vy += p * ny * 0.95
